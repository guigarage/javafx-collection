...
/**
 * Main class for the tutorial. This application will show only a simple wizard in the given {@link javafx.stage.Stage}. It's mainly
 ...
 * As shown in tutorial 1 and 2 the {@link io.datafx.controller.flow.Flow#startInStage(javafx.stage.Stage)} method of the Flow class is used to
 ...
 * of the {@link io.datafx.controller.flow.Flow} class.
 * In difference to the last tutorials a lot of actions are directly defined in the {@link io.datafx.controller.flow.Flow} definition instead of
 * defining them in the view controller classes. Therefore the {@link io.datafx.controller.flow.Flow} class provides a fluent API that can be used
 * to define actions for single views or the complete flow. All actions that are added to a flow have to implement the
 * {@link io.datafx.controller.flow.action.FlowAction} interface. The {@link io.datafx.controller.flow.Flow} class provides a lot of methods
 * to add specific action types the the flow. Examples are {@link io.datafx.controller.flow.Flow#withGlobalBackAction(String)} or
 *  {@link io.datafx.controller.flow.Flow#withLink(Class, String, Class)}. Some of them are used here to define the flow. As you can see in the
 ...
 *  actions, for example.
 */
public class WizardDemo extends Application {
